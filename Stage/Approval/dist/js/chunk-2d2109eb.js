...
// CONCATENATED MODULE: ./src/views/ApprovalChain/GroupList.vue?vue&type=template&id=46a54318&lang=en
    ...
    DeleteApprovalGroup(data) {
      ...
      vm.confirmR(vm.$t("ConfirmDelete"), true, vm.$t("Delete") + "  ", function () {
        ...
        DataService["a" /* default */].DeleteApprovalGroup(param).then(response => {
          if (response.data == '0') {
            vm.ShowAlert(vm.$t('ApprovalGroupDeletedSuccess'), "success", true, vm.$t("Alert"));
          } else if (response.data == '-1') {
            vm.ShowAlert(vm.$t('UnabletoDelete'), "warning", true, vm.$t("Alert"));
          }
          //vm.isLoading = false;
          vm.FetchData();
        });
